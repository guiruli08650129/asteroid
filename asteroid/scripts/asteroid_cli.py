...
def infer():
    ...
    parser.add_argument(
        "--ola-reorder",
        action="store_true",
        help="Whether to reorder each overlap-add chunk. See asteroid.dsp.LambdaOverlapAdd for details. Only used if --ola-window is set.",
    )
    parser.add_argument(
    ...
    if args.ola_window is not None:
        model = LambdaOverlapAdd(
            ...
            reorder_chunks=args.ola_reorder,
        )
