...
class LambdaOverlapAdd(torch.nn.Module):
    ...
    `LambdaOverlapAdd` works with `foo.separate` and the `asteroid-infer` CLI if
    the given `nnet` implements `foo.Separatable`. (Both `asteroid.models.BaseModel`
    and `asteroid.models.BaseEncoderMaskerDecoder` do.)
    ...
    def _separate(self, *args, **kwargs):
        return self.nnet._separate(*args, **kwargs)
